...
- **加／減頁（對頁）**：工具列「＋ 加一頁」「－ 減一頁」，字面一頁、實際一次 ±2(一個對頁、對齊偶數，電腦版兩頁書台不留半空白)。頁數存於 Firestore `meta/memorial.pages`（共享）。「減一頁」下限是 `maxPhotoPage+1`：收到最後一張照片的下一頁就 `disabled` 變灰、按不動，**有照片的頁刪不到**；`removePage` 內另有 `if (next === collagePages) return` 守門。頁數只是顯示用數字，照片的 `page/x/y` 分開存 `photos`，減頁不動照片資料，且 `effectivePages = max(collagePages, maxPhotoPage+1)` 永遠撐到最後一張照片所在頁——加頁回去排版照樣在。
...
### 鎖定延伸到前 32 頁 + 修復頁簽編碼亂碼（2026-06-25 已部署）
- **鎖定範圍 30 → 32**：粉絲在開放頁又補了照片，把鎖界線往後延一個對頁。`data.jsx` `LOCKED_PAGES = 32`、`firestore.rules` `lockedPages()` = `32`（兩邊一致）。
- **鎖定結果**：頁碼 3–34（`cindex 0–31`，底部對頁進度 2–17 / 18）照片＋排版鎖死；**頁碼 35–36（末攤 18/18，`cindex 32–33`）保持開放**給來不及的人補圖，該頁「加一頁」不會被鎖。
- **頁碼兩套對照**：每頁角落 `N / 36`（頁數）＋ 底部進度 `s / 18`（第幾個對頁）。對頁 s 的左右頁＝頁碼 `2s-1`、`2s`；故「對頁 17/18」＝頁碼 33、34。
- **修復頁簽亂碼**：`f1cfaa9`（鎖前 30 那筆）在對面機器被以非 UTF-8 存檔，`index.html` 標題 `心咲KOE — 紀念冊` 變亂碼、檔首多 BOM、註解破折號壞掉。改回乾淨 UTF-8（無 BOM）並還原標題與註解。
- `index.html` 全部 `?v=30` → `?v=31`。
- 已 `firebase deploy --only firestore:rules,hosting`。
...
---
_最後更新：2026-06-25（鎖定延伸到 LOCKED_PAGES=32 / 規則 lockedPages()=32，頁碼 3–34 鎖死、末攤 35–36 開放補圖；修復頁簽編碼亂碼；已部署）_
